...
bool_t webSocketIsRxReady(WebSocket *webSocket)
{
   ...
   if(webSocket->tlsContext != NULL)
   {
      ...
      if(webSocket->tlsContext->rxBufferLength > 0)
         available = TRUE;
   }
#endif
   ...
   while(webSocket->state != WS_STATE_CLOSED)
   {
      ...
      else if(webSocket->state == WS_STATE_CLOSING_TX)
      {
         ...
         else
         {
            ...
            if(webSocket->handshakeContext.closingFrameReceived)
               webSocket->state = WS_STATE_TLS_SHUTDOWN;
            else
               webSocket->state = WS_STATE_CLOSING_RX;
         }
      }
      else if(webSocket->state == WS_STATE_CLOSING_RX)
      {
         ...
         if(webSocket->handshakeContext.closingFrameReceived)
         {
            //Catch exception
            error = NO_ERROR;
            //Properly shutdown SSL/TLS session
            webSocket->state = WS_STATE_TLS_SHUTDOWN;
         }
      }
      else if(webSocket->state == WS_STATE_TLS_SHUTDOWN)
      {
#if (WEB_SOCKET_TLS_SUPPORT == ENABLED)
         //Check whether a secure connection is being used
         if(webSocket->tlsContext != NULL)
         {
            //Shut down the SSL/TLS session
            error = tlsShutdown(webSocket->tlsContext);
         }
#endif
         //Shut down the underlying TCP socket
         webSocket->state = WS_STATE_SOCKET_SHUTDOWN_TX;
      }
      else if(webSocket->state == WS_STATE_SOCKET_SHUTDOWN_TX)
      {
         //Further transmissions are disallowed
         error = socketShutdown(webSocket->socket, SOCKET_SD_SEND);
         //Next step
         webSocket->state = WS_STATE_SOCKET_SHUTDOWN_RX;
      }
      else if(webSocket->state == WS_STATE_SOCKET_SHUTDOWN_RX)
      {
