...
error_t mqttClientProcessPacket(MqttClientContext *context)
{
   ...
   case MQTT_PACKET_TYPE_PINGREQ:
      //Process incoming PINGREQ packet
      error = mqttClientProcessPingReq(context, dup, qos, retain, remainingLen);
      break;
   ...
   bool_t dup, MqttQosLevel qos, bool_t retain, size_t remainingLen)
{
   ...
   if(context->state == MQTT_CLIENT_STATE_CONNECT_SENT)
      context->state = MQTT_CLIENT_STATE_CONNACK_RECEIVED;
   ...
   bool_t dup, MqttQosLevel qos, bool_t retain, size_t remainingLen)
{
   ...
   if(context->publishCallback != NULL)
   {
      ...
      context->publishCallback(context, topic, message, messageLen);
   }
   ...
   if(qos == MQTT_QOS_LEVEL_1)
   {
      ...
      if(!error)
      {
         ...
         //Send PUBACK packet
         error = mqttClientSendData(context, context->packet, context->packetLen, NULL, 0);
      }
   }
   ...
   bool_t dup, MqttQosLevel qos, bool_t retain, size_t remainingLen)
{
   ...
   if(context->state == MQTT_CLIENT_STATE_PUBLISH_SENT)
      context->state = MQTT_CLIENT_STATE_PUBACK_RECEIVED;
   ...
   bool_t dup, MqttQosLevel qos, bool_t retain, size_t remainingLen)
{
   ...
   if(context->state == MQTT_CLIENT_STATE_PUBLISH_SENT)
      context->state = MQTT_CLIENT_STATE_PUBCOMP_RECEIVED;
   ...
   bool_t dup, MqttQosLevel qos, bool_t retain, size_t remainingLen)
{
   ...
   if(context->state == MQTT_CLIENT_STATE_SUBSCRIBE_SENT)
      context->state = MQTT_CLIENT_STATE_SUBACK_RECEIVED;
   ...
   bool_t dup, MqttQosLevel qos, bool_t retain, size_t remainingLen)
{
   ...
   if(context->state == MQTT_CLIENT_STATE_UNSUBSCRIBE_SENT)
      context->state = MQTT_CLIENT_STATE_UNSUBACK_RECEIVED;
...
/**
 * @brief Process incoming PINGREQ packet
 * @param[in] context Pointer to the MQTT client context
 * @param[in] dup DUP flag from the fixed header
 * @param[in] qos QoS field from the fixed header
 * @param[in] retain RETAIN flag from the fixed header
   ...
   bool_t cleanSession)
{
   ...
   if(context->protocolLevel == MQTT_PROTOCOL_LEVEL_3_1)
   {
   ...
   else if(context->protocolLevel == MQTT_PROTOCOL_LEVEL_3_1_1)
   {
      //The Protocol Name is a UTF-8 encoded string that represents the
      //protocol name "MQTT"
      error = mqttSerializeString(context->buffer, MQTT_CLIENT_BUFFER_SIZE,
   ...
   if(context->willMessage.topic[0] != '\0')
   {
      ...
      if(context->willMessage.qos == MQTT_QOS_LEVEL_1)
         connectFlags |= MQTT_CONNECT_FLAG_WILL_QOS_1;
      else if(context->willMessage.qos == MQTT_QOS_LEVEL_2)
   ...
   //packet payload
   error = mqttSerializeString(context->buffer, MQTT_CLIENT_BUFFER_SIZE,
      &n, context->clientId, strlen(context->clientId));
   ...
   if(context->willMessage.topic[0] != '\0')
   {
      ...
      error = mqttSerializeString(context->buffer, MQTT_CLIENT_BUFFER_SIZE,
         &n, context->willMessage.topic, strlen(context->willMessage.topic));
      ...
      error = mqttSerializeString(context->buffer, MQTT_CLIENT_BUFFER_SIZE,
         &n, context->willMessage.payload, context->willMessage.length);
   ...
   if(context->username[0] != '\0')
   {
      ...
      error = mqttSerializeString(context->buffer, MQTT_CLIENT_BUFFER_SIZE,
         &n, context->username, strlen(context->username));
   ...
   if(context->password[0] != '\0')
   {
      ...
      error = mqttSerializeString(context->buffer, MQTT_CLIENT_BUFFER_SIZE,
         &n, context->password, strlen(context->password));
